      ...
      fetch(apiUrl)
        ...
        .then((data) => {
          ...
          if (data && data.weather && data.weather.length > 0) {
            ...
            const formattedTime = `${hours}:${minutes}`; // Corrected template literal
    
            cityWeatherHTML += `<h4>Weather in ${cityData.name} at <span class="time">${formattedTime}</span>:</h4><p>${description}</p>`; // Corrected template literal
          } else {
            cityWeatherHTML += `<p>No weather description available for ${cityData.name}.</p>`; // Corrected template literal
          }
        ...
        .catch((error) => {
          ...
          allWeatherInfoHTML += `<div><p>Failed to fetch weather data for ${cityData.name}.</p></div><hr>`; // Corrected template literal
        })
        .finally(() => {
...
// This closing curly brace belongs to the forEach loop